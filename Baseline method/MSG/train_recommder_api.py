...
def decode_greedy(
    ...
    for di in range(100):
        ...
        #     # top_100_api_ids = topk.indices.squeeze(0).cpu().numpy()  # shape: (batch_size, 100)
        #     # top_100_api_names = [train_dataset.translate(row) for row in top_100_api_ids]  # list of list of names
        #
        ...
        #     # print(tgt.permute(1, 0)[:, 1:].cpu().numpy())
        #     # print(topk.indices.squeeze(0).cpu().numpy())
        #     data = { 'remove_apis': tgt.permute(1, 0)[:, 1:].cpu().numpy().tolist(),
        #              'predict_apis': topk.indices.squeeze(0).cpu().numpy().tolist()}
        #
