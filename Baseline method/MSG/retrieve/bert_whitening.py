...
MODEL_NAME = "bert-base-cased" # 本地模型文件
...
def main():
    ...
    oov_apis = []
    if oov is not None:
        with open('./data/api_name.json', 'r', encoding='utf-8') as f,open('./data/api_description.json', 'r', encoding='utf-8') as f2,open('./data/api_category.json', 'r', encoding='utf-8') as f3:
            apis = json.load(f)
            descs = json.load(f2)
            cats = json.load(f3)
