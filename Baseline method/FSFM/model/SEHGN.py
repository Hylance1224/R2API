...
class SelfAttention(nn.Module):
    def __init__(self, dim_embedding, dim_qk, dim_v):
        ...
        self.query = nn.Linear(dim_embedding, dim_qk)
        self.key = nn.Linear(dim_embedding, dim_qk)
        self.value = nn.Linear(dim_embedding, dim_v)
        self.scale = dim_qk ** 0.5  # 用于缩放点积结果

    def forward(self, x):
        """
        x: (batch_size, seq_len, dim_embedding)
        return: (batch_size, seq_len, dim_v)
        """
        Q = self.query(x)  # (batch_size, seq_len, dim_qk)
        K = self.key(x)  # (batch_size, seq_len, dim_qk)
        V = self.value(x)  # (batch_size, seq_len, dim_v)

        attn_weights = torch.bmm(Q, K.transpose(1, 2)) / self.scale  # (batch_size, seq_len, seq_len)
        attn_weights = f.softmax(attn_weights, dim=-1)

        out = torch.bmm(attn_weights, V)  # (batch_size, seq_len, dim_v)
        return out
...
class SEHGN(nn.Module):
    def __init__(self, config, idxs):
        ...
        # wyh self.api_api_affinity_graph = config.ds.mashup_ds.api_api_affinity_matrix
        ...
        # wyh self.api_api_affinity_mat = self.load_edge(self.api_api_affinity_graph, is_graph=False)
    ...
    def forward(self, mashup_des, user_indices, api_des, test=False):
        ...
        all_users, all_items = self.graph_comupte(graph=self.graph_mat, mess_dropout=self.mashup_api_level_dropout,
                                                  test=test)
        all_users_api_comp, all_items_api_comp = self.graph_comupte(graph=self.api_api_compatibility_mat,
                                                                     mess_dropout=self.api_api_compatibility_level_dropout,
                                                                     test=test,
                                                                     is_graph=False)
        # wyh all_users_api_affi, all_items_api_affi = self.graph_comupte(graph=self.api_api_affinity_mat,
        #                                                             mess_dropout=self.api_api_affinity_level_dropout,
        #                                                             test=test
        #                                                             , is_graph=False)
        ...
        all_items_api = (
                                self.api_api_compatibility_weight * all_items_api_comp ) / (
                                self.api_api_compatibility_weight)
    ...
    def graph_comupte(self, graph, mess_dropout, test, is_graph=True):
        ...
        for i in range(self.K):
            ...
            # 将稀疏图和特征移到 CPU 执行稀疏矩阵乘法
            graph_cpu = graph.cpu()
            all_emb_cpu = all_emb.cpu()

            # 稀疏矩阵乘法在 CPU 上执行
            all_emb_result = torch.sparse.mm(graph_cpu, all_emb_cpu)

            # 如果后续模型继续使用 GPU，再将结果放回 CUDA
            all_emb = all_emb_result.cuda() if all_emb.is_cuda else all_emb_result
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for batch_idx, batch_data in enumerate(data_iter):
                ...
                # api_target = batch_data[3].long().to(self.config.device)
    ...
    def case_analysis(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_data in enumerate(self.test_iter):
                ...
                api_target = [(sample != 0).nonzero(as_tuple=True)[0].tolist() for sample in batch_data[3]]
                api_pred_ = self.model(des, index, self.api_des)

                api_pred_ = api_pred_.cpu().argsort(descending=True)[:, :100].tolist()
